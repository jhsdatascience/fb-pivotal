        ...
        tabPanel('About',
            ...
            HTML("<a href = 'http://en.wikipedia.org/wiki/Fantasy_baseball'>Fantasy baseball</a> is a roster management game in which players (or <em>owners</em>) build teams of <a href = 'http://en.wikipedia.org/wiki/MLB'>Major League Baseball</a> players and compete based on various statistics produced by those MLB players. Typical statistics include <a href = 'http://en.wikipedia.org/wiki/Run_(baseball)'>Runs Scored</a>, <a href = 'http://en.wikipedia.org/wiki/Home_run'>Home Runs</a>, or <a href = 'http://en.wikipedia.org/wiki/Stolen_base'>Steals</a>. There are various formats for fantasy baseball leagues: they vary by the number of teams, by the statistical categories are used to determine points and by whether they are head-to-head or not.</p>
                ...
                <ul>
                    ...
                    <li>Match ups switch for the each week.</li>
...
<tr><td>The Edwin Encarnacions</td><td>52</td><td>18</td><td><strong>12</strong></td><td><strong>49</strong></td><td>6</td><td><strong>40</strong></td><td><strong>.303</strong></td><td>0</td></tr>
            ...
            HTML("<p>Now we come to the question of how to measure a player's contribution to a fantasy team. Call a player <em>pivotal</em> if for some week and some category a team wins that category if with the player on the roster but would not win that category if the player were replaced by a player of 'common skill' (see below).</p>"),
            h4("An example"),
            ...
            p("The plot should immediately update with a dark blue graph representing how the team would have performed without the player you just selected. The new graph may be discontinuous--if it is, that means that the player was not on the roster for the missing weeks."),
            p("You will also see the table below the graph update with the team's record without the selected player."),
            ...
            p("This quality is represented by the quantile at which the replacement level player performs. The default is .25, meaning that the player selected in the roster slot will be replaced by a player with stats in the 25th percentile in each category. By setting this quantile higher, you can see how the relative value of the player in question decreases . . ."),
            HTML("<img src = 'replacement-player-high.png'/>"),
            p('while by setting the quantile lower, you can see their relative value decrease:'),
            HTML("<img src = 'replacement-player-low.png'/>"),
            p('As mentioned above, the proper value for this quantile is not known.'),
            h2('Some final thoughts'),
            p('This app is simultaneously a proof of concept and a work in progress. If you have feedback, let me know. Some features I would like to add eventually:'),
            HTML("<ul>
                    <li>Pitchers!</li>
            ...
            h3('About the data'),
            p('The example data used for this project comes from a small fantasy baseball league of which I am a part. Many thanks to my league-mates for letting me crunch the numbers on our league and show them to the word.')
        )
    )
)